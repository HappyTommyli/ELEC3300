...
void LCD_DrawEllipse(uint16_t usC, uint16_t usP, uint16_t SR, uint16_t LR, uint16_t usColor)
{
	...
	uint16_t y = SR;
	// 2 QUADRANT
	for (uint16_t i = 0; i <= LR; i++)
	{
		for (uint16_t j = y; j >= 0; j--)
		{
			LCD_DrawDot(usC - i, usP + j, BLACK);
			LCD_DrawDot(usC - i, usP - j, BLACK);
			LCD_DrawDot(usC + i, usP + j, BLACK);
			LCD_DrawDot(usC + i, usP - j, BLACK);
			if (inside(SR, LR, i, j) == 1)
			{
				y = j;
				break;
			}
...
void LCD_Draw_picture(uint16_t usC, uint16_t usP,uint16_t h, uint16_t w, const uint16_t data [] )
  {
    int couter=0;
    LCD_OpenWindow(usC, usP,w, h);
    ...
    for (int i = 0;  i<h ; i++)
    {
      ...
      uint16_t mydata = data[couter];
      LCD_DrawDot(usC+j,usP+i,mydata);
      couter++;
  ...
  void draw_cchar(uint16_t usC, uint16_t usP,uint16_t h,uint16_t w, const unsigned char data[])
  {
    ...
    for (int ucPage = 0; ucPage < h * 3; ucPage++)
    {

      uint8_t mydata = data[ucPage];
      // one row need run 3 time,need three mydata
      for (int ucColumn = 0; ucColumn < 8; ucColumn++)
      {
        if (mydata & 0x01)
          LCD_Write_Data(0x001F);
	...
	void LCD_DrawString_promax(uint16_t usC, uint16_t usP, const char *pStr,uint8_t total_length){
	...
	if(count<total_length){
		LCD_Clear(usC,usP,8*(total_length-count),16,WHITE);
	}
